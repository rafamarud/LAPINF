...
1. Abra o **PowerShell como Administrador** e atualize o WSL:
   ```powershell
   wsl --update
   ```
2. Em seguida, force a atualização das políticas de grupo do Windows:
   ```powershell
   gpupdate /force
   ```
3. Reinicie a máquina caso qualquer um dos comandos solicite reinicialização.
...
*   **Nginx: Servidor**: Servidor web de alta performance que atua principalmente como proxy reverso e balanceador de carga. Ele fica na linha de frente do servidor recebendo as requisições externas dos usuários via HTTP/HTTPS, interceptando e entregando arquivos estáticos (como imagens, CSS e JavaScript) de forma ultra rápida. Quando a requisição exige processamento de código dinâmico, o Nginx a repassa de forma segura para o servidor de aplicação.
*   **Gunicorn (Green Unicorn)**: Servidor de aplicação HTTP para Python baseado no padrão WSGI (Web Server Gateway Interface). Ele funciona como o intermediário essencial (ou "canudo" de comunicação controlado) entre o servidor web Nginx e o código Python do framework Django. Sua principal função em produção é gerenciar múltiplos processos (workers) para executar a lógica de negócios do sistema SIEI de forma estável, performática e escalável.
